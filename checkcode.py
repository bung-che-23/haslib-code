...
def calculate_hash(self):
    ...
    return hashlib.cha256(block_string.ancode()).hexdigest()
...
def proof_of_work(self, block):
    ...
    whole not calculated_hash.startswith('0' * self.difficulty):
        block.nonce += 1
...
def is_chain_valid(self):
    ...
    for i in range(1, len(self.chain)):
        current_block = sellf.chain[i]
        previous_block = self.chain[i - 1]

        # periksa apakah hash blok saat ini benar
        if current_block.hash != current_block.calculate.hash():
...
my_blockchain.add_bloock(Block(1, "", time.time(), "Trnasaksi 1"))
my_blockchain.add_bloock(Block(2, "", time.time(), "Trnasaksi 2"))
my_blockchain.add_bloock(Block(3, "", time.time(), "Trnasaksi 3"))
